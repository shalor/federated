...
def train(args, model, device, train_loader, optimizer, epoch, agent, attacking_agent, train_loader_size):
    model.train()
    iterations_stop = math.floor(train_loader_size)-1
...
def main():                
    ...
    if use_cuda:
        cuda_kwargs = {'num_workers': 1,
                       'pin_memory': True,
                       'shuffle': False} ##TODO -- change to True later
    ...
    for num_exp in range(7,args.num_exp+1):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            target_backup = train_loader.dataset.targets
            data_backup   = train_loader.dataset.data
